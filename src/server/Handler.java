...
public abstract class Handler implements HttpHandler {
    ...
    protected final Gson gson = new GsonBuilder().registerTypeAdapter(Task.class, new TaskAdapter()).registerTypeAdapter(SubTask.class, new SubTaskAdapter()).registerTypeAdapter(Epic.class, new EpicAdapter()).setPrettyPrinting().serializeNulls().create();
    ...
    protected void handleGetRequest(HttpExchange exchange, String uriPath, Class<?> objectClass) throws IOException {
        ...
        if (uriPath.equalsIgnoreCase("/" + type.toLowerCase() + "s/")) {
            String json = switch (type) {
                case "Task" -> gson.toJson(manager.getTasks(), new TypeToken<HashMap<Integer, Task>>() {
                }.getType());
                case "Epic" -> gson.toJson(manager.getEpics(), new TypeToken<HashMap<Integer, Epic>>() {
                }.getType());
                case "SubTask" -> gson.toJson(manager.getSubTasks(), new TypeToken<HashMap<Integer, SubTask>>() {
                }.getType());
